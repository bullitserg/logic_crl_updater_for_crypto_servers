from ets.ets_certmanager_logs_parser import CertmanagerFile as Crt_f, get_info_file
...
logger.info('''Daemon started (search timeout: %s, drop timeout: %s)''' % (sleep_time, timeout))
...
def get_urls(_auth_key, _crl):
    ...
    # вызываем загрузчик
    download_crl(_auth_key, _crl)


def download_crl(_auth_key, _crl):
    ...
    if not urls:
        ...
        logger.info(log_add('''Url not found. Dropped''') % crl)
        return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            install_crl(_auth_key, _crl, crl_file_location)
            # и спокойно выходить
            return 0
...
def install_crl(_auth_key, _crl, file):
    ...
    if hash_cached_dict[_auth_key]['last_hash'] == hash_cached_dict[_auth_key]['actual_hash']:
        if status_cached_dict[_auth_key]['install_tries'] == max_install_tries:
            ...
            if status_cached_dict[_auth_key]['status']:
                logger.info(log_add('''Сrl was installed early (success). Passed''') % _crl)
            else:
                logger.info(log_add('''Сrl was installed early (error). Passed''') % _crl)
            return
...
if __name__ == '__main__':
    while True:
        ...
        # получаем списки словарей просроченных crl
        ...
        for crl in crl_overdue:
            ...
            if crl['AuthKeyID'] not in cached_id_keys:
                ...
                logger.info(log_add('''Repeat CRL updating...''') % crl)
        ...
        for key_id in crl_ok_inst_keys:
            url_cached_dict.pop(key_id)
            cached_dict.pop(key_id)
            hash_cached_dict.pop(key_id)
            status_cached_dict.pop(key_id)
        ...
        ignored_cached_dict.collect()
        url_cached_dict.collect()
        hash_cached_dict.collect()
        status_cached_dict.collect()
